...
function UserBody({ dataUser }) {
  ...
  return (
    <div>
      {/* Mostrar saldo pendiente solo si hay saldo */}
      {hasSaldo && (
        <PendingCard
          amount={dataUser.saldo}
          dataExpire={dataUser.fechaVenceSaldo}
        />
      )}
    </div>
  );
    ...
    if (session?.user?.email) {
      ...
      const fetchUserData = async (email) => {
        try {
          ...
          setDataUser(data.length > 0 ? data[0] : {});
        } catch (error) {
        ...
        >
          ...
          >
            <div className="space-y-6 md:space-y-10 mt-10">
              <h1 className="font-bold text-4xl text-center md:hidden"></h1>
              <h1 className="hidden md:block font-bold text-sm md:text-xl text-center">
                <span className="text-teal-600">Panel de Usuario</span>
              </h1>
              <CardUserSession
                name={dataSession?.name}
                email={dataSession?.email}
                image={dataSession?.image || null}
                role={"user"}
              />
              {/*Search bar */}
